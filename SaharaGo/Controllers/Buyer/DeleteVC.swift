...
class DeleteVC: UIViewController {
    ...
    @IBAction func onClikcBack(_ sender: Any) {
        self.navigationController?.popViewController(animated: true)
    }
    
    @IBAction func onClickPrivacy(_ sender: UIButton) {
        guard let url = URL(string: "https://craddle.com/privacy-policy") else {
          return //be safe
        }
        ...
        if Reachability.isConnectedToNetwork() {
            ...
            ServerClass.sharedInstance.postRequestWithUrlParameters(param, path: BASE_URL + PROJECT_URL.CHANGE_ACCOUNT_STATUS, successBlock: { (json) in
                ...
                if success == "true"
                {
                    ...
                    AnalyticsUtil.instance.trackEvent(AnalyticsEvents.kAccountDeleted)
                    isLogOut = "yes"
